...
class ResNet50():
    ...
    def __init__(self, number_classes, global_batch_size, img_size, learning_rate=3e-4, label_smoothing=0, use_l2_regularizer=True):
    ...
    def _build_model(self):
        ...
        # logits has shape: <batch_size>, 2048]
        # A softmax that is followed by the model loss must be done cannot be done
        # in float16 due to numeric issues. So we pass dtype=float32.
        softmax = tf.keras.layers.Softmax(axis=-1, name='softmax', dtype='float32')(logits)
        # softmax = tf.keras.layers.Activation('softmax', dtype='float32')(logits)

        resnet50 = tf.keras.Model(self.inputs, softmax, name='resnet50')
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape() as tape:
            ...
            loss_value = self.loss_fn(labels, softmax) # [Nx1]
