...
class ImageReader:
    ...
    def __init__(self, img_db, use_augmentation=True, balance_classes=False, shuffle=True, num_workers=1,
                 number_classes=2):
        random.seed()
    ...
    def __image_loader(self):
        ...
        try:
            ...
            while not termimation_flag:
                ...
                fM = np.zeros((self.nb_classes), dtype=np.int32)
                try:
                    fM[number] = 1
    ...
    def get_tf_dataset(self):
        ...
        label_shape = tf.TensorShape((self.nb_classes))
        return tf.data.Dataset.from_generator(self.generator, output_types=(tf.float32, tf.int32),
